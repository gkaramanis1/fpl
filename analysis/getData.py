...
def getNextWeekPicks():
    ...
    df_players = df_players[['chance_of_playing_next_round', 'chance_of_playing_this_round', 'code',
    ...
    df_players['image_url'] = [f"https://resources.premierleague.com/premierleague/photos/players/110x140/p{str(id)}.png" for id in df_players.code]
    ...
    for j, pl in players_sorted.iterrows():
        if (pl['position'] == 'Goalkeeper') & (sum(pk['position'] == 'Goalkeeper' for pk in next_week_picks) == 0):
            next_week_picks.append(pl.to_dict())
